...
<div class="app-header">
    <div style="padding-top: 5px;">
        ...
        <p style="margin:0; color: #64748b; font-size:15px;">Acompanhamento Estratégico de Performance, Custos e Auditoria de Frota — ANJUN</p>
    </div>
</div>
""", unsafe_allow_html=True)
...
with col1:
    if st.button(f"Gasto Total\n\nR$ {total_spend:,.2f}", key="kpi_spend"):
        st.session_state['global_clicked_quinzena'] = None
        st.session_state['global_clicked_placa'] = None
        st.session_state['global_clicked_motorista'] = None
        st.session_state['global_clicked_estado'] = None
...
with col4:
    if st.button(f"Média Geral\n\n{general_avg_km_l:.2f} km/L", key="kpi_avg"):
        ...
        st.rerun()
with col5:
    if st.button(f"Custo/KM Médio\n\nR$ {general_cost_km:.2f}/km", key="kpi_cost_km"):
        st.session_state['global_clicked_quinzena'] = None
        st.session_state['global_clicked_placa'] = None
        st.session_state['global_clicked_motorista'] = None
        st.session_state['global_clicked_estado'] = None
...
st.sidebar.markdown("### 🗺️ Menu do Painel")
...
# -----------------------------------------------------------------------------
if page == "📈 Custos & Volumes Quinzenais":
...
# -----------------------------------------------------------------------------
elif page == "🚚 Eficiência & Desempenho de Veículos":
    st.subheader("🚚 Eficiência Operacional e Desempenho Físico")
    
    # Charts Section (Rendered above table as requested!)
    ...
    with col_ef1:
        ...
        if not df_diesel_eff.empty:
            ...
            fig_eff = px.bar(
                ...
                text_auto='.2f',
                title='Comparativo de Consumo por Placa (Clique na barra para filtrar)',
                height=450
            )
...
# -----------------------------------------------------------------------------
elif page == "👨🏻‍✈️ Performance de Motoristas":
    ...
    with col_m1:
        ...
        fig_driver_spend = px.bar(
            ...
            title='Gastos Acumulados de Abastecimento por Quinzena',
            height=450
        )
...
# -----------------------------------------------------------------------------
# PAGE 5: AUDITORIA E BASE DE DADOS (INCONSISTENCIAS INTERATIVAS COM DRILLDOWN)
# -----------------------------------------------------------------------------
elif page == "🔍 Auditoria & Base de Dados":
